...
namespace ChatClient.Forms
{
    public partial class ChatFormNew : Form
    {
        ...
        //    SetupEvents();
        //}
        public ChatFormNew(User currentUser, SocketClientService socketClient)
        {
            _currentUser = currentUser;
            _socketClient = socketClient;   // dùng socket đã đăng nhập
        ...
        private void SetupForm()
        {
            ...
            // Security labels - hiển thị tất cả mức, VPD policy sẽ chặn nếu vượt clearance
            cbSecurityLabel.Items.Clear();
            for (int i = 1; i <= 5; i++)
            {
                var suffix = i > _currentUser.ClearanceLevel ? " ⚠️" : "";
                cbSecurityLabel.Items.Add($"Mức {i}{suffix}");
            }
