...
namespace ChatServer.Database
{
    public class DbContext : IDisposable
    {
        ...
        public async Task SetMacContextAsync(string username, int level)
        {
            using var cmd = Connection.CreateCommand();
            cmd.CommandText = "BEGIN SET_MAC_CONTEXT(:p_user, :p_level); END;";
        ...
        //    await cmd.ExecuteNonQueryAsync();
        //}
        //public async Task SetSessionUserLevelAsync(int level)
        //{
        //    using var cmd = Connection.CreateCommand();
        //    cmd.CommandText = "BEGIN MAC_CTX_PKG.SET_USER_LEVEL(:p_level); END;";
        //    cmd.CommandType = CommandType.Text;
        //    cmd.Parameters.Add(new OracleParameter("p_level", OracleDbType.Int32) { Value = level });
        ...
        SELECT 
            ...
            PASSWORD_HASH,
            MAVAITRO,
            NVL(CLEARANCELEVEL, 1),
            NVL(IS_BANNED_GLOBAL, 0),
            NVL(IS_OTP_VERIFIED, 0),
            PROFILE_NAME,
            NGAYTAO,
            LAST_LOGIN,
            LAST_LOGOUT,
        ...
        WHERE MATK = :p_value OR TENTK = :p_value";
            ...
            return new UserAccount
            {
                Matk = reader.IsDBNull(0) ? "" : reader.GetString(0),
                Username = reader.IsDBNull(1) ? "" : reader.GetString(1),
                PasswordHash = reader.IsDBNull(2) ? "" : reader.GetString(2),
                Mavaitro = reader.IsDBNull(3) ? "" : reader.GetString(3),

                ClearanceLevel = reader.GetInt32(4),
                IsBannedGlobal = reader.GetInt32(5) == 1,
                IsOtpVerified = reader.GetInt32(6) == 1,

                ProfileName = reader.IsDBNull(7) ? "" : reader.GetString(7),
                ...
                LockedUntil = reader.IsDBNull(14) ? null : reader.GetDateTime(14)
            };
        }
        ...
        public async Task<List<ConversationInfo>> GetUserConversationsAsync(string matkOrUsername)
        {
            ...
            // Set MAC context để VPD policy hoạt động đúng
            try
            {
                using var ctxCmd = Connection.CreateCommand();
                ctxCmd.CommandText = "BEGIN SET_MAC_CONTEXT(:p_matk); END;";
                ctxCmd.Parameters.Add(new OracleParameter("p_matk", OracleDbType.Varchar2) { Value = matk });
                await ctxCmd.ExecuteNonQueryAsync();
            }
            catch { /* Context might not exist, continue anyway */ }
            
            using var cmd = Connection.CreateCommand();
            // Query SELECT từ CUOCTROCHUYEN - VPD sẽ tự động filter:
            // - Có VPD_CUOCTROCHUYEN_SELECT: chỉ xem cuộc chat mình là thành viên (hoặc admin)
            // - Xóa VPD: xem tất cả cuộc trò chuyện
            cmd.CommandText = @"
        ...
        public async Task<List<AdminConversationInfo>> GetAllConversationsAsync()
        {
            ...
            cmd.CommandText = @"
                SELECT c.MACTC, c.TENCTC, c.MALOAICTC, c.IS_PRIVATE, c.NGUOIQL, c.NGAYTAO,
                       NVL(c.MIN_CLEARANCE, 1) AS MIN_CLEARANCE,
            ...
            while (await reader.ReadAsync())
            {
                result.Add(new AdminConversationInfo
                {
                    ...
                    MinClearance = reader.IsDBNull(6) ? 1 : reader.GetInt32(6),
                    MemberCount = reader.IsDBNull(7) ? 0 : reader.GetInt32(7),
        ...
        public async Task<List<BackupInfo>> GetBackupHistoryAsync(int limit = 100)
        {
            var backupHistory = new List<BackupInfo>();
            using var cmd = Connection.CreateCommand();
            cmd.CommandText = @"
            SELECT BACKUP_ID, BACKUP_TYPE, BACKUP_FILE, BACKUP_SIZE_MB, 
        ...
        // NEW: Get timeout settings
        public async Task<List<TimeoutSetting>> GetTimeoutSettingsAsync()
        {
            var settings = new List<TimeoutSetting>();
            using var cmd = Connection.CreateCommand();
            cmd.CommandText = @"
            SELECT SETTING_ID, SETTING_NAME, TIMEOUT_MINUTES, 
        ...
        public async Task<List<LoginHistoryInfo>> GetLoginHistoryAsync(int limit = 500)
        {
            var history = new List<LoginHistoryInfo>();
            using var cmd = Connection.CreateCommand();
            cmd.CommandText = @"
            SELECT LOGIN_TIME, USERNAME, HOVATEN, ACTION_TYPE, 
        ...
        // NEW: Execute backup procedure
        public async Task ExecuteBackupAsync(string backupType, string createdBy)
        {
            using var cmd = Connection.CreateCommand();
            cmd.CommandText = "SP_BACKUP_DATA";
            cmd.CommandType = CommandType.StoredProcedure;

            cmd.Parameters.Add(new OracleParameter("p_backup_type", OracleDbType.Varchar2) { Value = backupType });
            cmd.Parameters.Add(new OracleParameter("p_created_by", OracleDbType.Varchar2) { Value = createdBy });

            await cmd.ExecuteNonQueryAsync();
        }

        // NEW: Execute restore procedure
        public async Task ExecuteRestoreAsync(int backupId, string restoredBy)
        {
            using var cmd = Connection.CreateCommand();
            cmd.CommandText = "SP_RESTORE_BACKUP";
            cmd.CommandType = CommandType.StoredProcedure;

            cmd.Parameters.Add(new OracleParameter("p_backup_id", OracleDbType.Int32) { Value = backupId });
            cmd.Parameters.Add(new OracleParameter("p_restored_by", OracleDbType.Varchar2) { Value = restoredBy });

            await cmd.ExecuteNonQueryAsync();
        }

        // NEW: Update timeout setting
        public async Task UpdateTimeoutSettingAsync(string settingId, int timeoutMinutes, string updatedBy)
        {
            using var cmd = Connection.CreateCommand();
            cmd.CommandText = "SP_UPDATE_TIMEOUT_SETTING";
            cmd.CommandType = CommandType.StoredProcedure;

            cmd.Parameters.Add(new OracleParameter("p_setting_id", OracleDbType.Varchar2) { Value = settingId });
            cmd.Parameters.Add(new OracleParameter("p_timeout_minutes", OracleDbType.Int32) { Value = timeoutMinutes });
            cmd.Parameters.Add(new OracleParameter("p_updated_by", OracleDbType.Varchar2) { Value = updatedBy });
    ...
    public class AdminConversationInfo
    {
        ...
        public DateTime NgayTao { get; set; }
        public int MinClearance { get; set; } = 1;
        public int MemberCount { get; set; }
        public int MessageCount { get; set; }
    }
