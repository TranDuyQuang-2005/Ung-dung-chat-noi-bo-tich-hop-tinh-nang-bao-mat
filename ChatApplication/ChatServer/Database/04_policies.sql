...
-- Nếu không có context → cho xem tất cả (khi xóa VPD hoặc test)
CREATE OR REPLACE FUNCTION VPD_TINNHAN_SELECT_FN(
    ...
    v_matk VARCHAR2(100);
BEGIN
    v_matk := SYS_CONTEXT('MAC_CTX', 'USERNAME');
    
    -- Không có context → cho xem tất cả
    ...
    -- Top Secret (level 5) → xem tất cả
    IF v_user_level >= 5 THEN RETURN '1=1'; END IF;
    
    -- Filter theo MAC: No Read Up
...
-- User xem được cuộc chat nếu:
--   1. Admin (Clearance >= 4) → xem tất cả
--   2. Là thành viên của cuộc chat
...
BEGIN
    ...
    -- Nếu không có context, cho phép xem tất cả (fallback khi xóa VPD)
    IF v_matk IS NULL THEN 
        RETURN '1=1'; 
    END IF;
    
    -- Admin bypass (Clearance >= 4) → xem tất cả
    IF v_user_level >= 4 THEN 
        RETURN '1=1'; 
    END IF;
    
    -- User chỉ xem được cuộc chat mà mình là thành viên
    RETURN 'MACTC IN (SELECT MACTC FROM THANHVIEN WHERE MATK = ''' || v_matk || ''' AND DELETED_BY_MEMBER = 0)';
END;
/

-- VPD Policy Function cho TAIKHOAN
-- User xem được tài khoản nếu:
--   1. Admin (Clearance >= 4) → xem tất cả
--   2. CLEARANCELEVEL của tài khoản <= clearance của user
--   3. Hoặc là chính tài khoản của mình
...
BEGIN
    v_user_level := TO_NUMBER(NVL(SYS_CONTEXT('MAC_CTX', 'USER_LEVEL'), '1'));
    v_matk := SYS_CONTEXT('MAC_CTX', 'USERNAME'); -- Đây là MATK
    
    -- Không có context → cho xem tất cả
    IF v_matk IS NULL THEN RETURN '1=1'; END IF;
    
    -- Admin bypass
    IF v_user_level >= 4 THEN RETURN '1=1'; END IF;
    
    -- User xem được: tài khoản có clearance <= mình HOẶC là chính mình
    RETURN 'NVL(CLEARANCELEVEL,1) <= ' || v_user_level || ' OR MATK = ''' || v_matk || '''';
END;
/
...
BEGIN
    DBMS_RLS.ADD_POLICY(
        ...
        policy_function => 'VPD_TINNHAN_INSERT_FN', statement_types => 'INSERT', 
...
-- Disable trigger để test VPD
ALTER TRIGGER TRG_TINNHAN_CHECK_WRITE_UP DISABLE;
