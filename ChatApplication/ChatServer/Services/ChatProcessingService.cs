        ...
        private async Task<string> HandleLoginAsync(ChatRequest request)
{
    ...
    if (lockStatus.IsLocked)
    {
        ...
        return JsonSerializer.Serialize(new ServerResponse
        {
            ...
            Message = $"Tài khoản bị khóa tạm thời do nhập sai mật khẩu {lockStatus.FailedAttempts} lần. Vui lòng thử lại sau {remainingMinutes} phút hoặc liên hệ Admin."
                ...
                await _dbContext.SetMacContextAsync(request.SenderUsername, 5);
    ...
    catch (Oracle.ManagedDataAccess.Client.OracleException ex) when (ex.Number == 28110)
    {
        // ORA-28110: policy function or package has error
        Console.WriteLine($"[LOGIN] VPD Policy Error: {ex.Message}");
        return JsonSerializer.Serialize(new ServerResponse
        {
            Success = false,
            Message = "Lỗi cấu hình bảo mật database. Vui lòng liên hệ Admin."
        });
    }
    catch (Oracle.ManagedDataAccess.Client.OracleException ex)
    {
        Console.WriteLine($"[LOGIN] Database Error: {ex.Number} - {ex.Message}");
        return JsonSerializer.Serialize(new ServerResponse
        {
            Success = false,
        ...
        public async Task<string> HandleRequestAsync(string requestJson)
        {
            ...
            if (!string.IsNullOrEmpty(request.SenderUsername))
            {
                ...
                var actualClearance = account?.ClearanceLevel ?? 1;

                // *** FIX QUAN TRỌNG ***
                await _dbContext.SetSessionUserLevelAsync(request.SenderUsername, actualClearance);
            }
        ...
        //        // ==== QUAN TRỌNG: Set MAC clearance level đúng ====
        //        await _dbContext.SetSessionUserLevelAsync(account.ClearanceLevel);
        //    }

        //    // ====== CHECK ADMIN ACTION ======
        //    bool isAdminAction =
        //        request.Action.StartsWith("Admin") ||
        //        request.Action is "GetAllUsers" or
        //                            "GetUserDetails" or
        //                            "CreateUser" or
        //                            "UpdateUser" or
        //                            "DeleteUser" or
        //                            "BanUserGlobal" or
        //                            "UnbanUserGlobal" or
        //                            "GetAllConversations" or
        //                            "GetConversationMessagesAdmin" or
        //                            "AdminDeleteConversation" or
        //                            "AdminDeleteMessage" or
        //                            "GetAuditLogs";
        ...
        private async Task<string> HandleRegisterAsync(ChatRequest request)
        {
            ...
            try
            {
                ...
                try
                {
                    await _dbContext.CreateOtpAsync(matk, request.Email, otpHash);
                }
                catch (Exception ex)
                {
        ...
        private async Task<string> HandleVerifyOtpAsync(ChatRequest request)
        {
            ...
            var isValid = await _dbContext.VerifyOtpAsync(account.Matk, otpHash);

            if (isValid)
            {
                await _dbContext.WriteAuditLogAsync(account.Matk, "VERIFY_OTP_SUCCESS", request.SenderUsername, 0);
                return JsonSerializer.Serialize(new ServerResponse
                {
                    Success = true,
            ...
            await _dbContext.WriteAuditLogAsync(account.Matk, "VERIFY_OTP_FAILED", request.SenderUsername, 0);
            return JsonSerializer.Serialize(new ServerResponse
            {
                Success = false,
        ...
        private async Task<string> HandleForgotPasswordRequestAsync(ChatRequest request)
        {
            ...
            // SET MAC CONTEXT trước khi SELECT
            await _dbContext.SetSessionUserLevelAsync(request.SenderUsername, 5); // hoặc level từ param
            ...
            var storedEmail = await _dbContext.GetEmailByMatkAsync(account.Matk);
            ...
            var otp = PasswordHelper.GenerateOtp();
            var otpHash = PasswordHelper.HashPassword(otp);
            await _dbContext.CreateOtpAsync(account.Matk, request.Email, otpHash);
        ...
        private async Task<string> HandleResetPasswordAsync(ChatRequest request)
        {
            ...
            await _dbContext.UpdatePasswordAsync(account.Matk, newPasswordHash);

            await _dbContext.WriteAuditLogAsync(account.Matk, "RESET_PASSWORD", request.SenderUsername, 0);
        ...
        private async Task<string> HandleResendOtpAsync(ChatRequest request)
        {
            ...
            var email = await _dbContext.GetEmailByMatkAsync(account.Matk);
            ...
            var otp = PasswordHelper.GenerateOtp();
            var otpHash = PasswordHelper.HashPassword(otp);
            await _dbContext.CreateOtpAsync(account.Matk, email, otpHash);
        ...
        {
            // CanWrite check removed - VPD policy VPD_TINNHAN_INSERT sẽ enforce "No Write Up"
            // Nếu user gửi tin mức cao hơn clearance, database trigger sẽ raise error
            ...
            if (!string.IsNullOrEmpty(request.ConversationId))
            {
                ...
                {
                    // MAC Violation từ trigger TRG_TINNHAN_CHECK_WRITE_UP
                    Console.WriteLine($"[MAC] Blocked: {ex.Message}");
                    return JsonSerializer.Serialize(new ServerResponse
                    {
                        Success = false,
                        Message = $"⚠️ MAC Policy: Bạn không thể gửi tin nhắn mức {request.SecurityLabel} (Clearance của bạn: {request.ClearanceLevel})"
                    });
                }
                catch (Oracle.ManagedDataAccess.Client.OracleException ex)
                {
                    Console.WriteLine($"[DB Error] SendMessage: {ex.Message}");
                    return JsonSerializer.Serialize(new ServerResponse
                    {
                        Success = false,
                        Message = "Lỗi database khi gửi tin nhắn."
                    });
                }
            }
        ...
        private async Task<string> HandleGetMessagesAsync(ChatRequest request)
        {
            ...
            // VPD policy VPD_TINNHAN_SELECT sẽ tự động filter theo SecurityLabel
            var records = await _dbContext.GetMessagesForUserAsync(userAccount.Matk);

            var visible = records.Select(r => new ChatMessageDto
            {
        ...
        {
            if (string.IsNullOrEmpty(request.SenderUsername))
                return JsonSerializer.Serialize(new ServerResponse { Success = false, Message = "Username is required." });

            try
            {
                var account = await _dbContext.GetUserAccountAsync(request.SenderUsername);
                if (account == null)
                    return JsonSerializer.Serialize(new ServerResponse { Success = false, Message = "User not found." });

                // Parse JSON Content từ client
                var data = JsonSerializer.Deserialize<Dictionary<string, string>>(request.Content ?? "{}");

                string? hovaten = data.GetValueOrDefault("Hovaten");
                string? email = data.GetValueOrDefault("Email");
                string? sdt = data.GetValueOrDefault("Sdt");
                string? diachi = data.GetValueOrDefault("Diachi");
                string? bio = data.GetValueOrDefault("Bio");
                string? avatarUrl = data.GetValueOrDefault("AvatarUrl");

                DateTime? ngaysinh = null;
                if (data.TryGetValue("Ngaysinh", out var birthStr) && DateTime.TryParse(birthStr, out var parsed))
                {
                    ngaysinh = parsed;
                }

                await _dbContext.UpdateUserProfileFullAsync(
                    account.Matk,
                    hovaten,
                    email,
                    sdt,
        ...
        private async Task<string> HandleSendMessageWithAttachmentAsync(ChatRequest request)
        {
            ...
            // CanWrite check removed - VPD policy sẽ enforce "No Write Up"
            ...
            {
                // MAC Violation từ trigger TRG_TINNHAN_CHECK_WRITE_UP
                Console.WriteLine($"[MAC] Blocked attachment: {ex.Message}");
                return JsonSerializer.Serialize(new ServerResponse
                {
                    Success = false,
                    Message = $"⚠️ MAC Policy: Bạn không thể gửi tin nhắn mức {request.SecurityLabel} (Clearance của bạn: {request.ClearanceLevel})"
            ...
            {
                Console.WriteLine($"[Error] SendMessageWithAttachment: {ex.Message}");
                return JsonSerializer.Serialize(new ServerResponse
                {
                    Success = false,
        ...
        private async Task<string> HandleDownloadAttachmentAsync(ChatRequest request)
        {
            ...
            try
            {
                ...
                // CanRead check removed - VPD policy sẽ chặn access nếu user không có quyền
                ...
                Console.WriteLine($"[DOWNLOAD] Getting attachment for MessageId={request.MessageId}");
                var attachment = await _dbContext.GetAttachmentByMessageIdAsync(request.MessageId);
                if (attachment == null)
                {
                    Console.WriteLine($"[DOWNLOAD] ❌ No attachment found for MessageId={request.MessageId}");
                    return JsonSerializer.Serialize(new ServerResponse
                    {
                        Success = false,
                ...
                Console.WriteLine($"[DOWNLOAD] Found: AttachmentId={attachmentValue.AttachmentId}, FileName={attachmentValue.FileName}, DataLength={attachmentValue.Data?.Length ?? 0}");
                
                if (attachmentValue.Data == null || attachmentValue.Data.Length == 0)
                {
                    Console.WriteLine($"[DOWNLOAD] ❌ Attachment data is empty");
                    return JsonSerializer.Serialize(new ServerResponse
                    {
                        Success = false,
